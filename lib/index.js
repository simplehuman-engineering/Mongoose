 ...
 * Gets connection
 *
 * If a connection does not exist, creates a new one
 *
 * @param {string} name
 * @method get
 * @api public
 */
Mongoose.prototype.getConnection = function (name) {
    var found;

    this.savedConnections.forEach(function (c) {
        if (c.name == name) found = c.connection;
    })

    if (found) return found;
    var conn = { name: name, connection: this.createConnection() };
